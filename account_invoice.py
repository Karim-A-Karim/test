...
class account_invoice(models.Model):
    ...
    fuck_line = fields.One2many('x_fuck', 'x_account_invoice_id', string='fuck u',
        readonly=True, states={'draft': [('readonly', False)]}, copy=True)
    ...
    @api.multi
    def fucku(self):
        """ this function defined by karim to print the fucking AMT catering_invoice
        """
        assert len(self) == 1, 'This option should only be used for a single id at a time.'
        self.sent = True
        return self.env['report'].get_action(self, 'account.report_invoice_fucku')
